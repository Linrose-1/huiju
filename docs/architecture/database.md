...
状态：MySQL + Drizzle 首批 schema 与待执行 migration，2026-09-25。表定义位于 `apps/api/src/database/schema`，迁移位于 `apps/api/drizzle`；迁移尚未连接真实 MySQL 执行。产品行为以[首期 PRD](../product/首期PRD.md) F01–F04 为准。
...
所有业务时间按 UTC 写入，API 使用 ISO 8601，界面按中国时区展示。MySQL 连接的时区设为 UTC；事务比较使用服务端可信时间，不接受客户端提交的当前时间。外键、唯一键、索引和事务 SQL 在生成 migration 后审查，并用真实 MySQL 验证最后名额竞争、重复报名、取消释放与首次邀请绑定。

...
状态：2026-09-26。MySQL + Drizzle 首批表已定义；迁移 `0000`、`0001` 按 2026-09-25 工程记录已在隔离 MySQL 8.4.11 与主机开发库 8.0.36 执行验证。2026-09-26 先前任务已执行身份大小写修正迁移 `0002`；该修正在 8.4.11 上尚未验证。表定义位于 `apps/api/src/database/schema`，迁移位于 `apps/api/drizzle`。本次沿用现有结构实现会话、资料、活动浏览与报名；已通过真实 MySQL 8.0.36 单事务回滚验证，未执行新迁移。2026-09-26 17:36按用户授权直接使用本机开发库进行真实多连接并发测试，首次身份竞争、邀请固定、最后名额、重复报名、并发取消和再报通过；仅清理本次UUID/appId，前后9表数量一致。未建立独立测试库，微信上游仍为测试替身。产品行为以[首期 PRD](../product/首期PRD.md) F01–F04 为准。
